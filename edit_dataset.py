...
class EditDataset(Dataset):
    ...
    def __getitem__(self, i: int) -> dict[str, Any]:
        ...
        # task ='depth' 
        task = self.task
       # task = np.random.choice(['inv_seg', 'inv_depth', 'inv_hed','inv_canny', 'seg', 'depth', 'hed', 'canny','outpainting']) 
        ...
        if task == 'inv_seg':
            ...
            image_canny = Image.open(propt_dir.joinpath(f"{example_seed}_0_canny.jpg"))
            image_canny = image_canny.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_canny = 2 * torch.tensor(HWC3(np.array(image_canny))).float() / 255. - 1

            example_pair = torch.cat((image_canny, image_0), dim=2)  # h w c

            image_query = Image.open(propt_dir.joinpath(f"{seed}_1_canny.jpg"))
            image_query = image_query.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_query = 2 * torch.tensor(HWC3(np.array(image_query))).float() / 255. - 1
            ...
            example_pair = torch.cat((image_0, image_canny), dim=2)  # h w c
            image_query = image_1
            prompt = 'canny map'

            image_target = Image.open(propt_dir.joinpath(f"{seed}_1_canny.jpg"))
            image_target = image_target.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_target = 2 * torch.tensor(HWC3(np.array(image_target))).float() / 255. - 1
            
        elif task == 'outpainting':

            image_outpainting = Image.open(propt_dir.joinpath(f"{example_seed}_0_outpainting.jpg"))
            image_outpainting = image_outpainting.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_outpainting = 2 * torch.tensor(HWC3(np.array(image_outpainting))).float() / 255. - 1

            example_pair = torch.cat((image_outpainting, image_0), dim=2)  # h w c

            image_query = Image.open(propt_dir.joinpath(f"{seed}_1_outpainting.jpg"))
            image_query = image_query.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_query = 2 * torch.tensor(HWC3(np.array(image_query))).float() / 255. - 1
            ...
            image_normal = Image.open(propt_dir.joinpath(f"{example_seed}_0_normal.jpg"))
            image_normal = image_normal.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_normal = 2 * torch.tensor(HWC3(np.array(image_normal))).float() / 255. - 1

            example_pair = torch.cat((image_normal, image_0), dim=2)  # h w c

            image_query = Image.open(propt_dir.joinpath(f"{seed}_1_normal.jpg"))
            image_query = image_query.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_query = 2 * torch.tensor(HWC3(np.array(image_query))).float() / 255. - 1
            ...
            example_pair = torch.cat((image_0, image_normal), dim=2)  # h w c
            image_query = image_1
            prompt = 'normal map'

            image_target = Image.open(propt_dir.joinpath(f"{seed}_1_normal.jpg"))
            image_target = image_target.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
            image_target = 2 * torch.tensor(HWC3(np.array(image_target))).float() / 255. - 1

        # elif task == 'inpainting':

        #     image_inpainting = Image.open(propt_dir.joinpath(f"{example_seed}_0_inpainting.jpg"))
        #     image_inpainting = image_inpainting.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
        #     image_inpainting = 2 * torch.tensor(HWC3(np.array(image_inpainting))).float() / 255. - 1

        #     example_pair = torch.cat((image_inpainting, image_0), dim=2)  # h w c

        #     image_query = Image.open(propt_dir.joinpath(f"{seed}_1_inpainting.jpg"))
        #     image_query = image_query.resize((reize_res, reize_res), Image.Resampling.LANCZOS)
        #     image_query = 2 * torch.tensor(HWC3(np.array(image_query))).float() / 255. - 1
