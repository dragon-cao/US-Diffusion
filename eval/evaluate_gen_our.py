...
import random
import json
sys.path.append('/home/data_2t/cwl/Prompt-Diffusion2')
...
from eval import distributed as dist



def get_parser(**parser_kwargs):
    parser = argparse.ArgumentParser(**parser_kwargs)
    parser.add_argument("--image_dir", type=str, default='', help="path to saving original/generated images")
    parser.add_argument("--ckpt", type=str, default='', help="path to the saved ckpt")
    parser.add_argument("--task", type=str, default='inv_seg', help="task to test")
    return parser
...
def main():
    ...
    model = create_model('./models/cldm_v15_jiaocha.yaml').cpu()
    ...
    # Data
    dataset = EditDataset(path='/home/data_2t/cwl/Prompt-Diffusion2/data/clip-filtered-dataset',
                              split='test', prompt_option='output', task=opt.task)
    dataloader = DataLoader(dataset, batch_size=50, shuffle=False, num_workers=1, worker_init_fn=worker_init_fn)
    ...
    for i, batch in enumerate(dataloader):
        if i in batch_ids:
            ...
            un_cond = {"c_crossattn": [model.get_learned_conditioning([n_prompt] * len(aug_prompt))],
                       "query":        cond['query'],
                       "example_pair":     cond['example_pair']}
            ...
            query_images = (batch['query'] * 127.5 + 127.5).cpu().numpy().clip(0, 255).astype(np.uint8)

            for j, (y, x, query, prompt) in enumerate(zip(ori_images, x_samples, query_images, batch['txt'])):
                ...
                query_dir = os.path.join(opt.image_dir, opt.task, 'query_images')
                os.makedirs(query_dir, exist_ok=True)
                query_image_path = os.path.join(query_dir, f'{image_id}.png')
                Image.fromarray(query).save(query_image_path)

                filename = f'batch_{i}_{j}.json'
                path = os.path.join(opt.image_dir, opt.task, filename)
